...
const clearAll = () => {
  getContainer("svg g.lines").innerHTML = "";
  getContainer("svg g.points").innerHTML = "";
};

export class Figure {
  draw(config: FigureConfig) {
    clearAll();
    generateNCircles(config.pointTotal);
    generateNLines(config.multiplyFactor, config.pointTotal);
  }
}
